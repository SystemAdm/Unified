...
// Create a mock request with is_published=true but no author
$requestData = [
    'title' => $news->title,
...
echo "Mock request data (note: no author):\n";
print_r($requestData);
...
echo "Validated data without preserved author:\n";
print_r($validated);
...
echo "author would be: " . ($clonedNews->author ?? 'null') . "\n";
...
// Apply our fix: preserve the existing author
$validated['author'] = $news->author;

echo "Validated data with preserved author:\n";
print_r($validated);
...
if ($news->is_published && $news->isPublished() && $news->author === $originalState['author']) {
    echo coloredText("\nUpdate was successful! The article is now published and the author was preserved.\n", "green");
} else {
    ...
    if ($news->author !== $originalState['author']) {
        echo coloredText("- The author was not preserved. Original: {$originalState['author']}, Current: {$news->author}.\n", "red");
    }
}
...
$news->author = $originalState['author'];
$news->save();
...
echo "5. Verify that the author is preserved\n";
