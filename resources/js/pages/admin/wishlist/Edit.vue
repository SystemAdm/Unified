...
const form = useForm({
    ...
    link: props.wishlist.link || '',
    cost_per_unit: props.wishlist.cost_per_unit,
    count: props.wishlist.count,
    deadline: props.wishlist.deadline ? new Date(props.wishlist.deadline).toISOString().split('T')[0] : '',
    image: null,
    selected_image: props.wishlist.image || '',
});
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col space-y-6">
            <HeadingSmall title="Edit Wishlist Item" :description="`Editing: ${wishlist.name}`" />
            <form @submit.prevent="submit" class="max-w-2xl space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div v-if="wishlist.image" class="space-y-2">
                        <Label class="text-sm font-medium">Current Image</Label>
                        <div class="border rounded-lg p-4 max-w-xs">
                            <img :src="`/storage/${wishlist.image}`" :alt="wishlist.name" class="w-full h-auto rounded-lg" />
                        </div>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <p class="text-sm text-gray-500">Upload a new image for the wishlist item. Max size: 2MB.</p>
