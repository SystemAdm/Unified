...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: props.event.title, href: route('admin.events.edit', { event: props.event.id }) },
];

// Format date for form input (YYYY-MM-DDThh:mm)
...
// Determine initial signup options based on event data
const determineSignupStartOption = () => {
    if (!props.event.signup_start_date) return SIGNUP_START_OPTIONS.NOW;

    // If signup start date is the same as event start date
    if (props.event.signup_start_date === props.event.start_date) {
        return SIGNUP_START_OPTIONS.EVENT_START;
    }

    // If signup start date is in the past (assuming it was set to "now" when created)
    const now = new Date();
    const signupStart = new Date(props.event.signup_start_date);
    if (signupStart < now) {
        return SIGNUP_START_OPTIONS.NOW;
    }

    // Otherwise, it's a custom date
    return SIGNUP_START_OPTIONS.CUSTOM;
};

const determineSignupEndOption = () => {
    if (!props.event.signup_end_date) return SIGNUP_END_OPTIONS.EVENT_END;

    // If signup end date is the same as event start date
    if (props.event.signup_end_date === props.event.start_date) {
        return SIGNUP_END_OPTIONS.EVENT_START;
    }

    // If signup end date is the same as event end date
    if (props.event.signup_end_date === props.event.end_date) {
        return SIGNUP_END_OPTIONS.EVENT_END;
    }

    // Otherwise, it's a custom date
    return SIGNUP_END_OPTIONS.CUSTOM;
};
...
const form = useForm({
    ...
    organization_ids: selectedOrgIds.value,
});

// Watch for changes in signup options and update dates accordingly
watch([() => form.start_date, () => form.end_date, signupStartOption, signupEndOption], () => {
...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="flex flex-col space-y-6">
            <HeadingSmall title="Edit Event" :description="`Editing: ${props.event.title}`" />
            ...
            <form @submit.prevent="submit" class="space-y-6 max-w-xl">
                ...
                <div class="space-y-4 border p-4 rounded-md">
                    ...
                    <div class="space-y-2">
                        ...
                        <Input
                            id="class_restriction"
                            v-model="form.class_restriction"
                            type="text"
                            placeholder="Enter class restriction (optional)"
                        />
